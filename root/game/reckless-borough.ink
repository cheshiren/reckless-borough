...
{arm_red:
	...
	- 2: Видно, что восьмипалый протез с того фото — это правая рука мужчины.
}
...
=== function polaroid()
	...
	[table]<>
		[header]ИскИн - справка[/header]<>
		[row]Точно не оригинальный аппарат. Кустарная модификация с заменой и расширением исходного функционала: современной технологией моментальных снимков и прикреплением к ним мета-данных.[/row]<>
...
=== function aic()
	...
	[table]<>
		...
		[row]Животные с физическими и когнитивными приращениями использовались там, где это было более экономически целесообразно, чем применение полностью искусственных агентов с аналогичными возможностями: в разведке, саперных работах, поиске раненых и тому подобном.[/row]<>
...
=== function theft()
	...
	[table]<>
		...
		[row]Территория находится вне федеральной юрисдикции, поэтому данная отсылка — часть внутреннего культурного кода и криминальной традиции локальных преступных группировок.[/row]<>
	[/table]
=== function devanagari()
